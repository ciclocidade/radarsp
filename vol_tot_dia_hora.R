...
vol_tot_dia_hora <- function(
  ...
  if (is.null(dia_inicio) | is.null(dia_fim)) {
    ...
    if(length(files_to_read) == 0){
      ...
      by_day_loc_hora <- by_day_loc_hora %>% 
        rbind(nchar30_day_loc_hora)
      
      return(by_day_loc_hora)
    }
  }
}
