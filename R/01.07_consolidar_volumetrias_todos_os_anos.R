...
# TODO: Atualizar aqui
cods2015 <- data.frame(cod_local = names(vol2015))
...
cods2021 <- data.frame(cod_local = names(vol2021))
cods2022 <- data.frame(cod_local = names(vol2022))

# Criar um dataframe com todas as entradas únicas de cod_local
# TODO: Atualizar aqui
all_cods <-
  rbind(cods2015, cods2016, cods2017, cods2018, cods2019, cods2020, cods2021, cods2022) %>%
  distinct() %>%
  filter(str_detect(cod_local, '[0-9]{4}')) %>%
  arrange(cod_local)
...
for (cod in all_cods$cod_local) {
  ...
  # TODO: Atualizar aqui
  if (cod %in% names(vol2015)) {
    tmp_2015 <- vol2015 %>% select(data, all_of(cod))
  } else {
    tmp_2015 <- create_dummy_df('2015', cod)
  }
  ...
  if (cod %in% names(vol2021)) {
    tmp_2021 <- vol2021 %>% select(data, all_of(cod))
  } else {
    tmp_2021 <- create_dummy_df('2021', cod)
  }

  if (cod %in% names(vol2022)) {
    tmp_2022 <- vol2022 %>% select(data, all_of(cod))
  } else {
    tmp_2022 <- create_dummy_df('2022', cod)
  }

  # Dataframe temporário são os dias de todos os anos e respectivos volumes
  # TODO: Atualizar aqui
  tmp_all <- rbind(tmp_2015, tmp_2016, tmp_2017, tmp_2018, tmp_2019, tmp_2020, tmp_2021, tmp_2022)
  head(tmp_all)
...
# 2015
datas2015 <- vol2015 %>% select(data)
total2015 <- calcular_totais(vol2015)
total2015 <- cbind(datas2015, total2015)
...
# 2021
datas2021 <- vol2021 %>% select(data)
total2021 <- calcular_totais(vol2021)
total2021 <- cbind(datas2021, total2021)

# 2022
datas2022 <- vol2022 %>% select(data)
total2022 <- calcular_totais(vol2022)
total2022 <- cbind(datas2022, total2022)


# Juntar tudo
totais <- rbind(total2015, total2016, total2017, total2018, total2019, total2020, total2021, total2022)
