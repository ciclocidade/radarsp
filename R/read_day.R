...
#'
#' @description
#' Download speed camera records grouped by location and day. 
#'
...
#'                 as_data_frame = TRUE)
#'                 
read_day <- function(start = "2019/01/01",   # string, YYYY/MM/DD
    ...
    if (length(ids_not_found) > 0) {
      stop(
        paste0(
          ...
          " não foram encontrados. Use função 'radarsp::ddicionario_radarsp()' para abrir tabela de referência com os ids existentes."
        )
      )
    }
  }
